...
class Tetris:
    ...
    def reset(self,mode:str = None):
        ...
        self.gameover = False
        if mode=='ppo':
            board_tensor=torch.tensor(self.board,dtype=int,device=device)
            return board_tensor
        else:
    ...
    def get_next_states(self,mode:str = None):
        states = {}
        states_ppo={}
        piece_id = self.ind
        ...
        for i in range(num_rotations):
            ...
            for x in range(valid_xs + 1):
                ...
                board_tensor=torch.tensor(board,dtype=int,device=device)
                states_ppo[(x, i)] =board_tensor
                states[(x, i)] = self.get_state_properties(board)
            curr_piece = self.rotate(curr_piece)
        #print(f"piece_id:{piece_id}, action space: {len(states)}")
        if mode=='ppo':
            return states_ppo
        else:
            return states
    ...
    def get_ppo_state(self):
        return self.store(self.piece,self.current_pos)
    def new_piece(self):
        if not len(self.bag):
            self.bag = list(range(len(self.pieces)))
            random.shuffle(self.bag)
